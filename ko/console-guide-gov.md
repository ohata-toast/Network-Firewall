...
* Phase 1/2 설정
 ...
 > * Phase 2 추가는 최대 3개까지 가능합니다.
  > * Phase 2의 프라이빗 IP는 /24비트 이하로 설정하세요.
/24비트 이상의 값을 설정해야 할 경우 서브넷 범위를 사전에 확인하여 서브넷의 시작 IP로 입력하세요.
> [예시]
    1. 192.168.100.0/20 (X) → 192.168.96.0/20 (O)
    2. 172.16.30.0/21 (X) → 172.16.24.0/21 (O)
    3. 10.0.50.0/22 (X) → 10.0.48.0/22 (O)
